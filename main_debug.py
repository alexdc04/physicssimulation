...
for num in range (numOfJoints):
    if p.getJointInfo(agent, num)[2] != p.JOINT_FIXED:
        joints_dict[p.getJointInfo(agent, num)[0]]=(p.addUserDebugParameter(str(p.getJointInfo(agent, num)[1]),-1.5,1.5,0))
print(joints_dict)
positions={}
...
while True:
    ...
    print(p.getBasePositionAndOrientation(agent))
    p.setJointMotorControlArray(agent, joints_dict.keys(), controlMode=p.POSITION_CONTROL, targetPositions=positions.values())
    p.stepSimulation()
